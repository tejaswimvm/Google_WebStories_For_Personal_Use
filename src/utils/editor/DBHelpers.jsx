...
export const addStoryToDB = (storyObj) =>
  new Promise((resolve, reject) => {
    console.log("calling add to db", storyObj);
    const request = indexedDB.open(DB_NAME);
    request.onerror = (event) => {
      reject(event.target.errorCode);
    };
...
export const getStoryInDB = (storyId) =>
  new Promise((resolve, reject) => {
    ...
    request.onsuccess = (event) => {
      ...
      getRequest.onsuccess = (getEvent) => {
        console.log("getting by id ", getEvent.target.result?.featuredMedia);
        resolve(getEvent.target.result);
      };
    };
  });
 ...
 * @return {Promise}
 */
export const getFeaturedMediaUrl = (mediaId) =>
  new Promise((resolve, reject) => {
    const request = indexedDB.open(DB_NAME);
    request.onerror = (event) => {
      reject(event.target.errorCode);
    ...
    request.onsuccess = (event) => {
      ...
      getRequest.onsuccess = (getEvent) => {
        const featuredUrl = getEvent.target.result.find(
          (mediaItem) => mediaId === mediaItem.id
        );
        resolve(featuredUrl?.src);
      };
    };
  });
